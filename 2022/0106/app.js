...
// let shit = null
// let fuck = null
// const modifiedSentence = lyrics.split(/[ |,]/).map(word => {
//     return word.includes('shit') ? `${''}💩` : (word.includes('fuck') ? `${''}❤️` : word)}).join(' ')
...
// novel.split('').forEach((word)=>{
//         if(specialCharacters.indexOf(word) !==-1){
//             return
//             }    
//         else{
//             if(!classified[word.toLowerCase()]){
//             classified[word.toLowerCase()] = 0
//             }
//             classified[word.toLowerCase()]++
//         }
//     }
// )
// for(let key in classified){
//     const a = {alp : key, cnt : classified[key]}
//     alphabetFrequency.push(a)
// }

// console.log(classified)
// alphabetFrequency.sort((b,a)=> a.cnt-b.cnt)
...
function incluedWord(keyword){
    keywordDiv.innerHTML = keyword
    const findWord = lyrics.split(/[' '|,]/).map(word => {return word = word === keyword ? `${keywordDiv.textContent+'111'}`: word})
    console.log(keywordDiv)
    lyricsEl.classList.add('highlight')
    lyricsEl.innerHTML = findWord
